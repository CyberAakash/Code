...
def create_graph():
    ...
    for i in range(num_vertices):
        node_label = input(f"Enter the label for node {i + 1}: ")
        graph[node_label]  # Adding each vertex as a key to the graph dictionary

    num_edges = int(input("Enter the number of edges: "))
    for i in range(num_edges):
        edge_input = input(f"Enter edge {i + 1} (format: u v): ")
...
def add_edge(graph):
    u, v = input("Enter the edge to add (format: u v): ").split()
    graph[u].append(v)
    graph[v].append(u)

def add_node(graph):
    new_node_label = input("Enter the label for the new node: ")
    graph[new_node_label] = []  # Initialize the adjacency list for the new node

    # Update the adjacency lists of existing nodes to include the new node
    for node in graph:
        if node != new_node_label:  # Avoid connecting the new node to itself
            graph[node].append(new_node_label)
            graph[new_node_label].append(node)
...
def delete_node(graph):
    node_to_delete = input("Enter the label for the node to delete: ")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if choice == 1:
            ...
            goal_node = input("Enter the goal node for DFS: ")
